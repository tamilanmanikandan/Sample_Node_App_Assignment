...
pipeline {
    agent any
    environment {
        registry = "859058206093.dkr.ecr.us-east-1.amazonaws.com/courseassignment"
    }
    ...
    stage('Pushing to ECR') {
        steps{
            script {
                sh 'docker login -u AWS -p $(aws ecr get-login-password --region us-east-1) 859058206093.dkr.ecr.us-east-1.amazonaws.com/courseassignment '
                sh 'docker push 859058206093.dkr.ecr.us-east-1.amazonaws.com/courseassignment'
            }
        }
    }
